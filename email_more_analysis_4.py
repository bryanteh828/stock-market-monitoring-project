...
m,b = np.polyfit(np.array(df['trend_mean']),np.array(df['rsi14']),1)
plt.figure(figsize=(19,9))
plt.clf()
plt.subplot(1,2,1)
plt.title('Date: ' + df['date_of_analysis'].str.split(' ').str.get(0).max(),loc='left',fontweight='bold')
for i in df[group].unique():
    plt.scatter(df[df[group]==i]['trend_mean'],df[df[group]==i]['rsi14'],label=i,alpha=0.65)
plt.xlabel('Trend ~ 200MA of 5 years (range -1:1)',fontweight='bold')
plt.ylabel('Momentum ~ RSI-14 days (range 0:100)',fontweight='bold')

for x, y in zip( df['trend_mean'] ,df['rsi14']):
  label = list(df[(df['trend_mean']==x)&(df['rsi14']==y)]['name'])[-1]
  print(label)
  plt.annotate( label , (x,y) , textcoords ="offset points" , xytext=(0,8) , size = 7.5, ha='center')
  

plt.axhline(list(df[df['name']=='sp500']['rsi14'])[-1],color='green',alpha=0.13,linestyle='--')
plt.axvline(list(df[df['name']=='sp500']['trend_mean'])[-1],color='green',alpha=0.13,linestyle='--')
plt.axhline(list(df[df['name']=='hangseng']['rsi14'])[-1],color='green',alpha=0.13,linestyle='--')
...
sns.regplot(df['trend_mean'],df['rsi14'],x_ci=99,scatter=False, line_kws={'alpha': 0})
# sns.regplot(df['trend_mean'],df['rsi14'],x_ci='ci',scatter=False,color='black',line_kws={'alpha':0.01})
#################
m,b = np.polyfit(np.array(df['trend_mean']),np.array(df['Close_200ma']),1)


ax1 = plt.subplot(1,2,2)
for i in df[group].unique():
    plt.scatter(df[df[group]==i]['trend_mean'],df[df[group]==i]['Close_200ma'],label=i,alpha=0.65)
plt.xlabel('Trend ~ 200MA of 5 years (range -1:1)',fontweight='bold')
plt.ylabel('Discount ~ /200MA (range -1:1)',fontweight='bold')
    
plt.plot(df['trend_mean'],m*df['trend_mean']+b,label='fit',color='black',alpha=0.04)
plt.axhline(list(df[df['name']=='sp500']['Close_200ma'])[-1],color='green',alpha=0.13,linestyle='--')
plt.axvline(list(df[df['name']=='sp500']['trend_mean'])[-1],color='green',alpha=0.13,linestyle='--')
plt.axhline(list(df[df['name']=='hangseng']['Close_200ma'])[-1],color='green',alpha=0.13,linestyle='--')
plt.axvline(list(df[df['name']=='hangseng']['trend_mean'])[-1],color='green',alpha=0.13,linestyle='--')

sns.regplot(df['trend_mean'],df['Close_200ma'],x_ci=99,scatter=False, line_kws={'alpha': 0})
for x, y in zip( df['trend_mean'] ,df['Close_200ma']):
      label = list(df[(df['trend_mean']==x)&(df['Close_200ma']==y)]['name'])[-1]
      print(label)
